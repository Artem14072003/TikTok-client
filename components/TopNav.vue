...
onMounted(() => {
  document.addEventListener("mouseup", (e) => {
    ...
    if (!popupMenu?.contains(e.target as Node))
...
const isLoggedIn = () => {
  if ($userStore.id) {
    ...
    $generaleStore.isLoginOpen = true
  }
}
  ...
  >
    ...
    >
      <div :class="route.fullPath === '/' ? 'w-[80%]' : 'lg:w-[20%] w-[70%]'">
        <NuxtLink to="/">
          <img width="115" src="~/assets/image/tiktok-logo.png" alt="TicTok_logo">
        </NuxtLink>
      </div>
      ...
      <div class="flex items-center justify-end gap-3 min-w-[275px] max-w-[320px] w-full">
        <button
            @click="isLoggedIn()"
            class="flex items-center border rounded-sm px-3 py-[6px] transition-colors duration-300 ease-in hover:bg-gray-100"
        >
          <Icon name="mdi:plus" color="#000000" size="22"/>
          <span class="px-2 font-medium text-[15px]">Загрузить</span>
        </button>

        <div v-if="!$userStore?.id" class="flex items-center">
          <button
              @click="$generaleStore.isLoginOpen = true"
              class="flex items-center bg-[#F02C56] text-white border rounded-md px-3 py-[6px]"
          >
            <span class="mx-4 font-medium text-[15px]">Войти</span>
          </button>
        ...
        <div v-else class="flex items-center">
          ...
          <div class="relative">
            ...
            >
              ...
              >
                <Icon name="ph:user"/>
                <span class="pl-2 font-semibold text-sm">Профиль</span>
              </NuxtLink>
              <div
                  @click="logout()"
              ...
              >
                ...
                <span class="pl-2 font-semibold text-sm">Выйти</span>
              </div>
            </div>
          </div>
        </div>
